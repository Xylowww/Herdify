...
function AdminPage() {
  ...
  const filteredListings = listingStatusFilter ? mockListings.filter((l) => l.status === listingStatusFilter) : mockListings;
  const filteredUsers = userStatusFilter ? mockUsers.filter((u) => u.status === userStatusFilter) : mockUsers;
  const filteredReports = reportStatusFilter ? mockReports.filter((r) => r.status === reportStatusFilter) : mockReports;
  return <div>
      {
    /* Admin Header */
  }
    ...
    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm whitespace-nowrap transition-all duration-150 ${activeTab === tab.id ? "bg-[#2F6B3F] text-white shadow-sm" : "text-[#6B7280] hover:bg-[#F6F4EE] hover:text-[#1F2937]"}`}
  >
              <tab.icon size={15} />
              <span style={{ fontWeight: activeTab === tab.id ? 600 : 400 }}>{tab.label}</span>
              {tab.count !== void 0 && <span className={`px-1.5 py-0.5 rounded-full text-xs ${activeTab === tab.id ? "bg-white/20 text-white" : "bg-[#E2DDD5] text-[#6B7280]"}`} style={{ fontWeight: 600, fontSize: "10px" }}>
                  {tab.count}
                </span>}
            </button>)}
        </div>
  ...
  >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 shadow-sm">
                ...
                <div className="space-y-1.5 mt-2">
                  {pieData.map((d) => <div key={d.name} className="flex items-center justify-between text-xs">
    ...
    /* Pending Reports quick view */
            ...
            <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 shadow-sm">
              ...
              {mockReports.filter((r) => r.status === "Pending").map((report) => <div key={report.id} className="flex items-center gap-3 p-3 rounded-xl bg-amber-50 border border-amber-200 mb-2 hover:bg-amber-100/80 transition-colors duration-150">
    ...
    /* -- Listings -- */
        ...
        {activeTab === "listings" && <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[#1F2937]" style={{ fontWeight: 600 }}>All Listings</h3>
              <div className="flex gap-1.5">
                {["", "Available", "Reserved", "Sold", "Removed"].map((s) => <button
    key={s}
  ...
  >
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#F6F4EE]">
                  {filteredListings.map((l) => <tr key={l.id} className="table-row-hover">
    ...
    /* -- Users -- */
        ...
        {activeTab === "users" && <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[#1F2937]" style={{ fontWeight: 600 }}>User Management</h3>
              <div className="flex gap-1.5">
                {["", "Active", "Suspended", "Pending"].map((s) => <button key={s} onClick={() => setUserStatusFilter(s)} className={`px-2.5 py-1.5 rounded-lg text-xs transition-all duration-150 ${userStatusFilter === s ? "bg-[#2F6B3F] text-white shadow-sm" : "text-[#6B7280] hover:bg-[#F6F4EE]"}`} style={{ fontWeight: 500 }}>
                    {s || "All"}
                  </button>)}
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#F6F4EE]">
                  {filteredUsers.map((u) => <tr key={u.id} className="table-row-hover">
                      <td className="py-3.5 pr-4">
                        <div className="flex items-center gap-2.5">
                      ...
                      <td className="py-3.5 pr-4">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${u.role === "Admin" ? "bg-purple-100 text-purple-800" : u.role === "Seller" ? "bg-[#F0F7F2] text-[#2F6B3F]" : "bg-blue-100 text-blue-800"}`} style={{ fontWeight: 600 }}>{u.role}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-xs text-[#6B7280]">{formatDate(u.joinDate)}</td>
                      <td className="py-3.5 pr-4">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${u.status === "Active" ? "bg-green-100 text-green-800" : u.status === "Suspended" ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800"}`} style={{ fontWeight: 600 }}>{u.status}</span>
                      </td>
                      <td className="py-3.5">
                        <div className="flex items-center gap-1">
                          {u.status !== "Suspended" ? <button onClick={() => toast.success(`${u.name} suspended.`)} className="p-1.5 rounded-lg text-[#D64545] hover:bg-red-50 transition-all duration-150 active:scale-90" title="Suspend">
                              <Ban size={14} />
                            </button> : <button onClick={() => toast.success(`${u.name} reactivated.`)} className="p-1.5 rounded-lg text-[#2F6B3F] hover:bg-[#F0F7F2] transition-all duration-150 active:scale-90" title="Reactivate">
                              <RefreshCw size={14} />
                            </button>}
                          <button className="p-1.5 rounded-lg text-[#6B7280] hover:bg-[#F6F4EE] transition-all duration-150 active:scale-90" title="View">
                            <Eye size={14} />
    ...
    /* -- Reports -- */
        ...
        {activeTab === "reports" && <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[#1F2937]" style={{ fontWeight: 600 }}>Reports</h3>
              <div className="flex gap-1.5">
                {["", "Pending", "Reviewed", "Resolved"].map((s) => <button key={s} onClick={() => setReportStatusFilter(s)} className={`px-2.5 py-1.5 rounded-lg text-xs transition-all duration-150 ${reportStatusFilter === s ? "bg-[#2F6B3F] text-white shadow-sm" : "text-[#6B7280] hover:bg-[#F6F4EE]"}`} style={{ fontWeight: 500 }}>
                    {s || "All"}
                  </button>)}
              </div>
            </div>
            <div className="space-y-3">
              {filteredReports.map((r) => <div key={r.id} className="p-4 rounded-xl border border-[#E2DDD5] hover:border-[#2F6B3F]/30 hover:shadow-sm transition-all duration-150">
    ...
    /* -- Audit Logs -- */
        ...
        {activeTab === "audit" && <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#F6F4EE]">
                  {mockAuditLogs.map((log) => <tr key={log.id} className="table-row-hover">
                      <td className="py-3.5 pr-4">
