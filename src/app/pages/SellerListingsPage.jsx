...
function SellerListingsPage() {
  ...
  >
    ...
    className={`px-3 py-1.5 rounded-xl border text-sm whitespace-nowrap transition-all duration-150 flex-shrink-0 ${statusFilter === s ? "bg-[#2F6B3F] text-white border-[#2F6B3F] shadow-sm" : "border-[#E2DDD5] text-[#6B7280] hover:border-[#2F6B3F]/50 hover:text-[#2F6B3F]"}`}
    style={{ fontWeight: 500 }}
  >
            {s || "All"}
            {s && <span className="ml-1.5 text-xs opacity-75">
    ...
    /* Listings Grid */
      ...
      {filtered.length === 0 ? <div className="text-center py-20 bg-white rounded-2xl border border-[#E2DDD5] shadow-sm empty-state">
  ...
  >
                    {["Available", "Reserved", "Sold", "Removed"].map((s) => <option key={s} value={s}>{s}</option>)}
                  </select>
                  <Link
