    ...
    return mockListings.filter((l) => {
      ...
      if (filters.search) {
        ...
        if (!l.title.toLowerCase().includes(q) &&
            ...
            !l.type.toLowerCase().includes(q)) return false;
      }
      return true;
    });
  }, [filters]);
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-4 mt-3">
              {[
                ...
                <span key={item.text} className="hidden md:flex items-center gap-1.5 text-white/70 text-xs">
                  <item.icon size={12} />
                  {item.text}
                </span>
              ))}
          ...
          <div className="flex items-center gap-1 glass p-1 rounded-xl shadow-soft">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm transition-all duration-300 active:scale-95 ${
                !mapView ? 'bg-gradient-to-r from-[#2F6B3F] to-[#3a834d] text-white shadow-glow' : 'text-white/80 hover:text-[#2F6B3F] hover:bg-white/50'
              }`}
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0">
          {mapView ? (
            ...
            >
              {filtered.length === 0 ? (
                <div className="glass-card flex flex-col items-center justify-center py-28 text-center">
