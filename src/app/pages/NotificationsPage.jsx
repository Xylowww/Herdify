...
const NotifIcon = ({ type }) => {
  ...
  return <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-soft" style={{ background: bg }}>
  ...
  >
    ...
    /* Notifications */
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        {notifications.length === 0 ? <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          </div> : <div className="divide-y divide-[#E2DDD5]/50">
            {notifications.map((notif, i) => <motion.button
    key={notif.id}
    initial={{ opacity: 0, x: -12 }}
    animate={{ opacity: 1, x: 0 }}
    transition={{ delay: i * 0.03, duration: 0.3, ease: [0.4, 0, 0.2, 1] }}
    onClick={() => markNotificationRead(notif.id)}
    className={`w-full text-left flex items-start gap-3 px-5 py-4 hover:bg-white/50 transition-all duration-300 ${!notif.read ? "bg-gradient-to-r from-[#F0F7F2]/50 to-transparent border-l-3 border-l-[#2F6B3F]" : ""}`}
  >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {!notif.read && <span className="w-2.5 h-2.5 rounded-full bg-gradient-to-br from-[#2F6B3F] to-[#3a834d] flex-shrink-0 shadow-glow" />}
                    </div>
                  </div>
                  <p className="text-sm text-[#6B7280] mt-0.5" style={{ lineHeight: 1.5 }}>{notif.body}</p>
                  {notif.link && <span className="text-xs text-[#2F6B3F] mt-1.5 inline-block hover:underline" style={{ fontWeight: 500 }}>
                      View details →
                    </span>}
                </div>
              </motion.button>)}
