...
import {
  ChevronLeft, Upload, MapPin, Lock, AlertCircle, ShieldCheck,
  Image as ImageIcon, X, ToggleLeft, ToggleRight, Info
} from 'lucide-react';
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 shadow-sm">
          ...
          <div
            ...
            onDrop={(e) => { e.preventDefault(); setIsDragging(false); addDemoPhoto(); }}
            className={`border-2 border-dashed rounded-xl p-8 text-center transition-all duration-200 cursor-pointer ${
              isDragging ? 'border-[#2F6B3F] bg-[#F0F7F2] scale-[1.01] shadow-sm' : 'border-[#E2DDD5] hover:border-[#2F6B3F]/50 hover:bg-[#FAFAF8]'
            }`}
          ...
          {photos.length > 0 && (
            <div className="flex gap-3 mt-3 flex-wrap">
              {photos.map((p, i) => (
                <div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border border-[#E2DDD5]">
                  ...
                  {i === 0 && (
                    <span className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-center py-0.5" style={{ fontSize: '9px', fontWeight: 600 }}>
                      COVER
                    </span>
                  )}
                </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 shadow-sm">
          ...
          <div className="p-4 bg-[#F6F4EE] rounded-xl border border-[#E2DDD5]">
            ...
            <p className="text-xs text-[#6B7280]" style={{ lineHeight: 1.5 }}>
              {locationPrivacy
                ? 'Approximate location shown (e.g., "Batangas City area"). Exact address revealed only after you approve a buyer inquiry.'
                : 'Full address visible to all users. Not recommended for privacy.'
              }
            </p>
