...
function SellerDashboardPage() {
  ...
  >
    ...
    /* Verification Banner */
      ...
      {!currentUser?.verified && <div className="flex items-center gap-3 p-4 glass-card rounded-xl mb-6" style={{ background: "linear-gradient(135deg, rgba(251,191,36,0.1), rgba(245,158,11,0.05))" }}>
      ...
      {currentUser?.verified && <div className="flex items-center gap-2 p-3.5 glass-card rounded-xl mb-6" style={{ background: "linear-gradient(135deg, rgba(47,107,63,0.08), rgba(111,175,95,0.05))" }}>
  ...
  />
    ...
    /* Recent Inquiries */
        ...
        <div className="glass-card rounded-2xl p-6 min-h-[280px]">
          ...
          <div className="space-y-3">
            {myConversations.length === 0 ? <p className="text-sm text-[#6B7280] text-center py-4">No inquiries yet</p> : myConversations.map((c) => <Link key={c.id} to={`/messages/${c.id}`} className="flex items-center gap-3 p-2.5 rounded-xl glass hover:shadow-soft transition-all duration-300">
                  ...
                  {c.unread > 0 && <span className="w-5 h-5 bg-gradient-to-br from-[#2F6B3F] to-[#3a834d] text-white rounded-full flex items-center justify-center text-xs shadow-glow" style={{ fontWeight: 600 }}>
                      {c.unread}
                    </span>}
                </Link>)}
          </div>
    ...
    /* My Listings Table */
      ...
      <div className="mt-6 glass-card rounded-2xl p-6">
        ...
        {myListings.length === 0 ? <div className="text-center py-10">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-[#E2DDD5]/30">
                {myListings.map((listing) => <tr key={listing.id} className="group hover:bg-white/50 transition-colors duration-200">
