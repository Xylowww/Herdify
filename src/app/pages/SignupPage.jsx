...
function SignupPage() {
  ...
  ].map((t) => <div key={t} className="flex items-start gap-2 text-white/80 text-sm">
  ...
  >
    ...
    /* Role Selection */
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {[
    { value: "Buyer", icon: ShoppingCart, desc: "Browse & purchase livestock" },
    { value: "Seller", icon: Package, desc: "List & sell your livestock" }
  ].map((r) => <button
    key={r.value}
    type="button"
    onClick={() => setRole(r.value)}
    className={`p-4 rounded-xl border-2 text-left transition-all ${role === r.value ? "border-[#2F6B3F] bg-[#F0F7F2] shadow-sm" : "border-[#E2DDD5] bg-white hover:border-[#2F6B3F]/40"}`}
  >
