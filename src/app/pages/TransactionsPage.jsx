...
import { mockTransactions, TRANSACTION_STATUS_COLORS } from "../data/mockData";
...
function TransactionsPage() {
  ...
  const myTransactions = mockTransactions.filter(
    (t) => t.sellerId === currentUser?.id || t.buyerId === currentUser?.id
  );
  const filtered = statusFilter ? myTransactions.filter((t) => t.status === statusFilter) : myTransactions;
  const totalCompleted = myTransactions.filter((t) => t.status === "Completed").reduce((s, t) => s + t.amount, 0);
      ...
      <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 shadow-sm">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex gap-1.5">
            {["", "Completed", "Pending", "Cancelled"].map((status) => <button
    key={status}
    onClick={() => setStatusFilter(status)}
    className={`px-3 py-1.5 rounded-lg text-xs transition-all duration-150 ${statusFilter === status ? "bg-[#2F6B3F] text-white shadow-sm" : "text-[#6B7280] hover:bg-[#F6F4EE]"}`}
  ...
  >
    ...
    >
                      ...
                      <td className="py-3.5">
                        <span className={`inline-flex px-2 py-1 rounded-full text-xs ${TRANSACTION_STATUS_COLORS[txn.status]}`} style={{ fontWeight: 600 }}>
                          {txn.status}
                        </span>
                      </td>
                    </motion.tr>;
