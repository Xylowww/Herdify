  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl border border-[#E2DDD5] overflow-hidden flex flex-col md:flex-row shadow-sm" style={{ minHeight: 560 }}>
        ...
        <div className={`w-full md:w-72 flex-shrink-0 border-r border-[#E2DDD5] flex flex-col ${!showConvList ? 'hidden md:flex' : 'flex'}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {mockConversations.length === 0 ? (
              <div className="text-center py-10 text-[#6B7280] text-sm px-4">No conversations yet</div>
            ) : (
              mockConversations.map((conv) => (
                <button
                  key={conv.id}
                  onClick={() => handleSelectConv(conv.id)}
                  className={`w-full text-left px-3 py-3 border-b border-[#F0EDE8] hover:bg-[#F6F4EE]/80 transition-colors duration-150 ${
                    selectedConvId === conv.id ? 'bg-[#F0F7F2] border-l-2 border-l-[#2F6B3F]' : ''
                  }`}
                >
                  <div className="flex items-start gap-2.5">
                    <div className="relative flex-shrink-0">
                      ...
                      />
                      {conv.unread > 0 && (
                        <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#D64545] text-white rounded-full flex items-center justify-center" style={{ fontSize: '9px', fontWeight: 700 }}>
                          {conv.unread}
                        </span>
                      )}
                    </div>
        ...
        {/* Message Area */}
        {conversation ? (
          <div className={`flex-1 flex flex-col min-w-0 ${showConvList ? 'hidden md:flex' : 'flex'}`}>
            {/* Chat Header */}
            <div className="px-3 sm:px-4 py-3 border-b border-[#E2DDD5] flex items-center justify-between gap-2 bg-[#FAFAF8]">
              ...
              <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                ...
                {conversation.contactShared ? (
                  <span className="hidden sm:flex items-center gap-1.5 text-xs text-[#2F6B3F] bg-[#F0F7F2] px-2 py-1 rounded-full border border-[#C8E0CF]" style={{ fontWeight: 500 }}>
                    <UserCheck size={12} />
                    Contact Shared
                  </span>
                ) : (
                  <button className="hidden sm:flex items-center gap-1.5 text-xs text-[#C68A3A] bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-full hover:bg-amber-100 transition-colors duration-150" style={{ fontWeight: 500 }}>
                    <UserCheck size={12} />
                    Share Contact
                  </button>
                )}
                      ...
                      <div className={`max-w-[75%] sm:max-w-xs lg:max-w-sm ${isMe ? 'items-end' : 'items-start'} flex flex-col gap-1.5`}>
                        <div
                          className={`px-3.5 py-2.5 rounded-2xl text-sm ${
                            isMe
                              ? 'bg-[#2F6B3F] text-white rounded-br-lg'
                              : 'bg-[#F6F4EE] text-[#1F2937] rounded-bl-lg border border-[#E2DDD5]/50'
                          }`}
          ...
          <div className={`flex-1 flex items-center justify-center text-center px-6 ${showConvList ? 'hidden md:flex' : 'flex'}`}>
            <div>
