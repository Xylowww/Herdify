...
function BottomNav() {
  ...
  return <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 glass-nav shadow-elevated">
      <div className="flex items-center justify-around h-14 px-1 pb-[env(safe-area-inset-bottom)]">
        {navItems.map((item) => <NavLink
    key={item.to}
    to={item.to}
    end={item.end}
    className={({ isActive }) => `relative flex flex-col items-center justify-center flex-1 h-full transition-all duration-300 active:scale-95 ${item.accent && !isActive ? "text-[#2F6B3F]" : isActive ? "text-[#2F6B3F]" : "text-[#9CA3AF]"}`}
  >
            {({ isActive }) => <>
                {isActive && <motion.div
  ...
  />}
                <div className="relative">
                  {item.accent && !isActive ? <div className="w-9 h-9 bg-gradient-to-br from-[#2F6B3F] to-[#3a834d] rounded-xl flex items-center justify-center -mt-1 shadow-glow">
  ...
  >
                ...
                <span
    className={`text-[10px] mt-0.5 transition-all duration-300 ${isActive ? "font-semibold" : "font-normal"}`}
  >
                  {item.label}
                </span>
              </>}
