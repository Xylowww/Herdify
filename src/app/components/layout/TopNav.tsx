import {
  Bell, Search, ChevronDown, LogOut, User, Settings, Shield,
  LayoutDashboard, MessageSquare, ClipboardList, PlusSquare,
  History, BarChart2, AlertTriangle, Users, FileText,
  Home, X
} from 'lucide-react';
...
export function TopNav() {
  ...
  const handleRoleSwitch = (newRole: 'Buyer' | 'Seller' | 'Admin') => {
    switchRole(newRole);
    setProfileOpen(false);
  ...
  return (
    <header className="h-14 glass-nav sticky top-0 z-40 shadow-soft">
      <div className="h-full max-w-[1600px] 2xl:max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 flex items-center">
        ...
        <nav className="hidden md:flex items-center justify-center flex-1 mx-2 lg:mx-4 gap-1">
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `relative flex items-center justify-center w-16 lg:w-24 h-11 rounded-xl transition-all duration-300 group ${
                  isActive
                    ? 'text-[#2F6B3F] bg-[#F0F7F2]/80'
                    : 'text-[#6B7280] hover:bg-white/60 hover:text-[#1F2937] hover:shadow-soft'
                }`
              }
            >
              {({ isActive }) => (
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0 ml-auto md:ml-0">
          {currentUser && (
            <span className="hidden sm:inline-flex items-center px-2.5 py-1 rounded-full text-xs glass text-[#2F6B3F]" style={{ fontWeight: 600 }}>
              {currentUser.role}
            </span>
          )}
          ...
          <div className="relative" ref={profileRef}>
            <button
              ...
              className={`flex items-center gap-1.5 p-1 pr-2 rounded-full transition-all duration-300 ${
                profileOpen ? 'glass ring-2 ring-[#2F6B3F]/20 shadow-soft' : 'hover:glass hover:shadow-soft'
              }`}
            >
              <img
                      ...
                      {(['Buyer', 'Seller', 'Admin'] as const).map((r) => (
                        <button
                          ...
                          className={`flex-1 py-1.5 rounded-lg text-xs transition-all duration-200 ${
                            currentUser?.role === r
                              ? 'bg-gradient-to-br from-[#2F6B3F] to-[#3a834d] text-white shadow-glow'
                              : 'glass text-[#6B7280] hover:text-[#1F2937] hover:shadow-soft'
                          }`}
