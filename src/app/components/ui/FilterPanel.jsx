...
function FilterPanel() {
  ...
  />
        {filters.search && <button onClick={() => setFilter("search", "")} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6B7280] hover:text-[#1F2937] transition-colors duration-150">
            <X size={14} />
          </button>}
      </div>
  ...
  >
    ...
    className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-sm transition-all duration-200 active:scale-95 ${expanded ? "border-[#2F6B3F] bg-[#2F6B3F] text-white" : "border-[#E2DDD5] text-[#6B7280] hover:border-[#2F6B3F]/40 hover:text-[#2F6B3F]"}`}
  >
    ...
    /* Advanced filters */
      ...
      {expanded && <div className="space-y-3 pt-2 border-t border-[#F0EDE8]">
          {
    /* Price range */
  }
          <div>
